...
const fallbackAgentCommandDescriptions = new Map<string, string>([
	["craft", "Full confirmed-brief-then-build flow. Runs multi-round shape discovery first, resolves visual probe and north-star mock gates when available, then builds and visually iterates. Use when building a new feature end-to-end."],
	["init", "Sets up a project for Impeccable. Runs a discovery interview when context is missing, writes PRODUCT.md, offers DESIGN.md when code exists, pre-configures live mode, and recommends next commands."],
	["document", "Generate a DESIGN.md file that captures the current visual design system: colors, typography, spacing, radii, and component patterns."],
	["extract", "Pull reusable patterns, components, and design tokens into the design system. Use when drift across the codebase should be consolidated."],
	["adapt", "Adapt designs to work across different screen sizes, devices, contexts, or platforms. Implements breakpoints, fluid layouts, and touch targets."],
	...
	["audit", "Run technical quality checks across accessibility, performance, theming, responsive design, and anti-patterns with severity-rated findings."],
	["bolder", "Amplify safe or boring designs to make them more visually interesting and stimulating while maintaining usability."],
	["clarify", "Improve unclear UX copy, error messages, microcopy, labels, and instructions so interfaces are easier to understand."],
	["colorize", "Add strategic color to monochromatic or flat UIs, making interfaces more engaging and expressive."],
	["critique", "Evaluate design from a UX perspective: hierarchy, IA, emotional resonance, cognitive load, scoring, anti-patterns, and actionable feedback."],
	["delight", "Add moments of joy, personality, and unexpected touches that make interfaces memorable and enjoyable to use."],
	["distill", "Strip designs to their essence by removing unnecessary complexity and noise."],
	["harden", "Make interfaces production-ready: error handling, i18n, text overflow, edge cases, and resilience under real-world data."],
	["onboard", "Design onboarding flows, first-run experiences, and empty states that guide new users to value."],
	["layout", "Improve layout, spacing, and visual rhythm. Fix monotonous grids, inconsistent spacing, weak hierarchy, and alignment issues."],
	["optimize", "Diagnose and fix UI performance across loading speed, rendering, animations, images, and bundle size."],
	["overdrive", "Push interfaces past conventional limits with technically ambitious implementations such as shaders, spring physics, and scroll-driven reveals."],
	["polish", "Perform a final quality pass fixing alignment, spacing, consistency, and micro-detail issues before shipping."],
	["quieter", "Tone down visually aggressive or overstimulating designs, reducing intensity while preserving quality."],
	["shape", "Plan UX and UI before code with discovery, visual probes when available, and a user-confirmed design brief."],
	["typeset", "Improve typography by fixing font choices, hierarchy, sizing, weight, and readability so text feels intentional."],
	["pin", "Create a standalone shortcut so a command can invoke the matching Impeccable command directly."],
...
export function parseCommandMetadata(json: string) {
	...
	if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) return descriptions;
